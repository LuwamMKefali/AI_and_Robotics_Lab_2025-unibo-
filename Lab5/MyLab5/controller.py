...
from robot_gwy import execute_door_command, pick_up_box, put_down_box
...
class GoTo (Behavior):
    ...
    def update_state(self, state):
        print("Behavior GoTo not imlemented yet: skipped")
...
class Cross (Behavior):
    ...
    def update_state(self, state):
        print("Behavior Cross not imlemented yet: skipped")
...
class Open (Behavior):
    """
    Open a door by a given name, just calling the relevant ROS2 service
    """
    def __init__(self, door):
        print("Instantiated Open behavior with door =", door)
        self.doorname = door
        super().__init__()
    
    def update_state(self, state):
        execute_door_command(self.doorname,"Open")
