...
# Optional parameter overrides via environment variables (useful for Task 2 "bad" experiment)
# Example:
#   export LAB1_WHEEL_RADIUS_OVERRIDE=0.110
#   export LAB1_WHEEL_AXIS_OVERRIDE=0.470
wr_over = os.environ.get("LAB1_WHEEL_RADIUS_OVERRIDE", None)
wa_over = os.environ.get("LAB1_WHEEL_AXIS_OVERRIDE", None)

if wr_over is not None:
    try:
        parameters['wheel_radius'] = float(wr_over)
        print("WARNING: overriding wheel_radius to", parameters['wheel_radius'])
    except:
        print("WARNING: invalid LAB1_WHEEL_RADIUS_OVERRIDE, ignoring:", wr_over)

if wa_over is not None:
    try:
        parameters['wheel_axis'] = float(wa_over)
        print("WARNING: overriding wheel_axis to", parameters['wheel_axis'])
    except:
        print("WARNING: invalid LAB1_WHEEL_AXIS_OVERRIDE, ignoring:", wa_over)
